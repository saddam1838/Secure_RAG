...
from .dependencies import get_current_user, get_admin_user, create_access_token, verify_password
...
async def react_login(req: LoginRequest):
    ...
    # 🔥 Load user's data into local memory (FAISS + corpus) for fast access
    global rag
    rag.load_user_data(req.username)
...
async def upload_and_index(file: UploadFile = File(...), user: str = Depends(get_current_user)):
    ...
    try:
        ...
        # Document metadata saving is handled by rag.add_document() below.
        if not report["is_safe"]:
            audit.log(user, "document_blocked", {
                "filename": file.filename,
                "issues": [i["name"] for i in report["issues"]],
                "size_mb": round(size_mb, 2)
            })
            raise HTTPException(400, f"Document blocked: {[i['name'] for i in report['issues']]}")
        
        # Document is safe - index it
...
async def delete_document(doc_id: str, user: str = Depends(get_current_user)):
    """Delete document and all associated chunks from vector store."""
    from services.cloud_storage import cloud_storage
    ...
    if cloud_storage.is_cloud_enabled:
        try:
            res = cloud_storage.supabase.table("documents").select("filename").eq("id", doc_id).eq("uploaded_by", user).execute()
            if res.data:
                filename = res.data[0]["filename"]
                print(f"🗑️ Deleting document: {filename} (ID: {doc_id})")
...
async def scan_document_only(file: UploadFile = File(...), user: str = Depends(get_current_user)):
    """Step 1: Scan document for threats WITHOUT indexing. Saves blocked docs to DB for audit."""
